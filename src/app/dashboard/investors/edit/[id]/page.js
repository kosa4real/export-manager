  ...
  const fetchInvestor = async () => {
    try {
      ...
      setFormData({
        ...
        contactInfo: investorData.contactInfo || "",
        email: investorData.email || "",
        investmentDate: investorData.investmentDate
          ? new Date(investorData.investmentDate).toISOString().split("T")[0]
          : "",
        amountInvested: investorData.amountInvested?.toString() || "",
        currency: investorData.currency || "USD",
        bankName: investorData.bankName || "",
        amountReceived: investorData.amountReceived?.toString() || "",
        exchangeRate: investorData.exchangeRate?.toString() || "",
        profitShare: investorData.profitShare || "",
        containerEquivalent: investorData.containerEquivalent?.toString() || "",
        status: investorData.status || "ACTIVE",
        isActive: investorData.isActive ?? true,
        notes: investorData.notes || "",
        userId: investorData.user?.id?.toString() || "",
      });
    } catch (err) {
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-400">Loading investor data...</p>
        </div>
      </div>
    );
  }
