...
export default function EditExportPage({ params }) {
  ...
  if (error && !formData.exportDate) {
    return (
      <div className="p-6 max-w-4xl mx-auto text-white">
        ...
        <button
          onClick={() => router.back()}
          className="mt-4 px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg font-medium transition-colors"
        >
          Go Back
        </button>
      </div>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto text-white">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Edit Export</h1>
        <p className="text-slate-400">Update export shipment record #{id}</p>
      </div>
