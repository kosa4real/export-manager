...
export default function EditSupplyExportPage() {
  ...
  const fetchSupplyExport = useCallback(async () => {
    try {
      ...
      setFormData({
        supplyId: data.supplyId.toString(),
        exportId: data.exportId.toString(),
        quantityBags: data.quantityBags.toString(),
      });
    } catch (err) {
      setError("Error fetching supply-export details");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to update supply-export");
      }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 bg-clip-text text-transparent mb-2">
            Edit Supply-Export Mapping
          </h1>
          <p className="text-slate-400 text-sm">
            Update the mapping between supply and export shipment
          </p>
        </div>
